@echo off
echo Checking Dapr status (not waiting for full health)...

REM Give Dapr a bit more time to initialize
timeout /t 30

REM Check if Dapr pods are at least running (even if not fully healthy)
kubectl get pods -n dapr-system

echo.
echo Proceeding with application deployment regardless of Dapr health status...
...
echo.
echo To check the status of your pods: kubectl get pods
echo To check the status of Dapr: dapr status -k
echo To get the frontend URL: minikube service todo-frontend-service --url
echo.
echo Note: Dapr components may still be initializing. This can take several minutes.
echo Monitor the Dapr pods with: kubectl get pods -n dapr-system
echo.
pause